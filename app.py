...
if st.button("Predict Loan Status"):
    ...
    # Bias metrics on original training set
    metric_before = BinaryLabelDatasetMetric(dataset_orig_train,
                    unprivileged_groups=[{'Gender': 0}],
                    privileged_groups=[{'Gender': 1}])

    metric_after = BinaryLabelDatasetMetric(dataset_transf_train,
                    unprivileged_groups=[{'Gender': 0}],
                    privileged_groups=[{'Gender': 1}])

    before_spd = metric_before.statistical_parity_difference()
    after_spd = metric_after.statistical_parity_difference()

    # Show bias comparison chart
    st.subheader("📊 Statistical Parity Difference (SPD)")
    st.write("Closer to 0 = less bias")
    fig, ax = plt.subplots()
    ax.bar(["Before Mitigation", "After Mitigation"], [before_spd, after_spd], color=['red', 'green'])
    ax.set_ylabel("SPD")
    st.pyplot(fig)
